...
Vector2* createFood(size_t count) {
    ...
    for (size_t i = 0; i < count; i++) {
        Vector2 pos = {random(1, GetScreenWidth()), random(1, GetScreenHeight()) };
...
void eatFood(Vehicle* vehic, Vector2* food, bool is_badFood) {
    ...
    if (idx > -1) {
        if (vec2_dist(vehic->location, food[idx]) < 3) {
            ...
            DrawCircleV(food[idx], 10, RAYWHITE);
...
int main() {
    ...
    Vector2* food = createFood(20);
    Vector2* poison = createFood(20);
    ...
    while (!WindowShouldClose()) {
        ...
        updateVehicle(&mover);
        food = respawnFood(food,100, 10);
        poison = respawnFood(poison, 100, 80);
        zahl = random(0, 1000);
        if (zahl < 10) {
            printf("%d", zahl);
        }
